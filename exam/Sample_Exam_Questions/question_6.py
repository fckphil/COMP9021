...
def f(for_seed, n, upper_bound):
    ...
    for i in range(len(square)):
        for j in range(len(square[0])):
            times_of_number[square[i][j]] = times_of_number.get(square[i][j], 0) + 1
    for key in times_of_number:
        set2.add(key) #prepared if is good square, set2 contains all number
        if times_of_number[key] != 1:
            duplicates.add(key)
    set_of_number = sorted(set2)
    col = []
    array = []
    number = 1
    for i in set_of_number:
        if number % n != 0:
            ...
            col = []
            number += 1
    transfer_array = zip(*array) #transfer col and row
    transfer_array = list(transfer_array)
    for i in range(len(transfer_array)):
        transfer_array[i] = list(transfer_array[i])
    ordered_square = transfer_array
